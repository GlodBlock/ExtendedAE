---
navigation:
    parent: epp_intro/epp_intro-index.md
    title: ME Mod Storage Bus
    icon: extendedae:mod_storage_bus
categories:
- extended devices
item_ids:
- extendedae:mod_storage_bus
---

# ME Mod Storage Bus

<GameScene zoom="8" background="transparent">
  <ImportStructure src="../structure/cable_mod_storage_bus.snbt"></ImportStructure>
</GameScene>

The ME Mod Storage Bus is an <ItemLink id="ae2:storage_bus" /> that can be filtered by mod name or mod ID.

Use commas to separate multiple mod IDs when filtering for multiple mods.

![PIC](../pic/mod_bus_name.png)

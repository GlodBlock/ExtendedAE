...
public class HighlightRender {
    ...
    public static void tick(PoseStack stack, MultiBufferSource multiBuf, Camera camera) {
    ...
    private static void drawBlockOutline(BlockPos pos, PoseStack stack, Camera camera, MultiBufferSource multiBuf) {
        ...
        if (camera.isInitialized()) {
            var vec3 = camera.getPosition().reverse();
            var aabb = (new AABB(pos)).move(vec3);
    ...
    private static void renderBox(MultiBufferSource multiBuf, PoseStack stack, Vec3 topLeft, Vec3 bottomLeft, Vec3 topRight, Vec3 bottomRight, float r, float g, float b, float a) {
    ...
    private static void renderLine(VertexConsumer buf, PoseStack pose, Vec3 from, Vec3 to, float r, float g, float b, float a) {

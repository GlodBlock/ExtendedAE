package com.github.glodblock.eterminal.client.gui;
...
import com.github.glodblock.eterminal.client.button.HighlightButton;
...
public class GuiExPatternTerminal extends AEBaseScreen<ContainerExPatternTerminal> {
    ...
    public GuiExPatternTerminal(ContainerExPatternTerminal menu, Inventory playerInventory,
        ...
        this.searchOutField.setTooltipMessage(Collections.singletonList(Component.translatable("gui.eterminal.ex_pattern_access_terminal.tooltip.01")));
        ...
        this.searchInField.setTooltipMessage(Collections.singletonList(Component.translatable("gui.eterminal.ex_pattern_access_terminal.tooltip.02")));
        ...
        for (; i < this.visibleRows; ++i) {
            if (scrollLevel + i < this.rows.size()) {
                ...
                if (highlightBtns.containsKey(scrollLevel + i)) {
                    ...
                    btn.x = this.leftPos + GUI_PADDING_X - SLOT_SIZE;
                    btn.y = this.topPos + (i + 1) * SLOT_SIZE + 34;
                ...
                if (row instanceof SlotsRow slotsRow) {
                    ...
                    for (int col = 0; col < slotsRow.slots; col++) {
                        ...
                        if (!this.searchOutField.getValue().isEmpty() || !this.searchInField.getValue().isEmpty()) {
                            if (this.matchedStack.contains(slot.getItem())) {
                                fillRect(poseStack, new Rect2i(slot.x, slot.y, 16, 16), 0x8A00FF00);
                            } else if (!this.matchedProvider.contains(container)) {
                                fillRect(poseStack, new Rect2i(slot.x, slot.y, 16, 16), 0x6A000000);
                            }
                        }
                    }
                    ...
                    if (group.icon() != null) {
                        var renderContext = new SimpleRenderContext(LytRect.empty(), poseStack, LightDarkMode.LIGHT_MODE);
                    ...
                    this.font.draw(poseStack, text, GUI_PADDING_X + PATTERN_PROVIDER_NAME_MARGIN_X + 10, GUI_PADDING_Y + GUI_HEADER_HEIGHT + i * ROW_HEIGHT, textColor);
                }
            }
        }
    }
        ...
        if (hoveredSlot == null) {
            ...
            if (hoveredLineIndex != -1) {
                ...
                if (row instanceof GroupHeaderRow headerRow && !headerRow.group.tooltip().isEmpty()) {
                    this.renderTooltip(poseStack, headerRow.group.tooltip(), Optional.empty(), x, y);
                    return;
                }
            }
        }
    ...
    public void drawBG(PoseStack poseStack, int offsetX, int offsetY, int mouseX, int mouseY, float partialTicks) {
        this.bindTexture("guis/ex_pattern_access_terminal.png");
        ...
        for (var group : this.groups) {
            ...
            for (var container : containers) {
                ...
                if (inventory.size() > 0) {
                    ...
                    btn.setMessage(Component.translatable("gui.eterminal.ex_pattern_access_terminal.tooltip.03"));
    ...
    private void blit(PoseStack poseStack, int offsetX, int offsetY, Rect2i srcRect) {
        this.blit(poseStack, offsetX, offsetY, srcRect.getX(), srcRect.getY(), srcRect.getWidth(), srcRect.getHeight());
    }

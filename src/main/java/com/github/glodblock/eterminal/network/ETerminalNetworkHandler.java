package com.github.glodblock.eterminal.network;

import appeng.core.sync.network.TargetPoint;
import com.github.glodblock.eterminal.EnhancedTerminal;
import com.github.glodblock.eterminal.network.packet.IMessage;
import com.github.glodblock.eterminal.network.packet.SExPatternInfo;
...
public class ETerminalNetworkHandler {

    private static final ResourceLocation channel = EnhancedTerminal.id("network");
    static int id = 0;

    public static final ETerminalNetworkHandler INSTANCE = new ETerminalNetworkHandler();
    ...
    public ETerminalNetworkHandler() {
        ...
        this.clientHandler = DistExecutor.unsafeRunForDist(() -> () -> ETerminalNetworkHandler::onClientPacketData,
                () -> () -> pkt -> {
                });
    }
            ...
            ctx.enqueueWork(
                    () -> {
                        try {
                            ...
                            EnhancedTerminal.LOGGER.warn(e.getMessage());
                        }
                    });
    ...
    public void sendToAll(IMessage<?> message) {
        var server = EnhancedTerminal.INSTANCE.getServer();
        if (server != null) {
    ...
    public Packet<?> toFMLPacket(IMessage<?> message, NetworkDirection direction) {
        ...
        if (id == -1) {
            EnhancedTerminal.LOGGER.error(String.format("Unregistered Packet: %s", message.getPacketClass()));
        }

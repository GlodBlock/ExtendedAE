...
public class CalculatorAssemblerMatrix extends MBCalculator<TileAssemblerMatrixBase, ClusterAssemblerMatrix> {
    ...
    public boolean checkMultiblockScale(BlockPos min, BlockPos max) {
        if (max.getX() - min.getX() > EAEConfig.assemblerMatrixSize) {
            return false;
        }
        if (max.getY() - min.getY() > EAEConfig.assemblerMatrixSize) {
            return false;
        }
        if (max.getZ() - min.getZ() > EAEConfig.assemblerMatrixSize) {
            return false;
        }

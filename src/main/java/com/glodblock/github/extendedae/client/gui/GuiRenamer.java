...
public class GuiRenamer extends AEBaseScreen<ContainerRenamer> implements IActionHolder {
    ...
    public GuiRenamer(ContainerRenamer menu, Inventory playerInventory, Component title, ScreenStyle style) {
        ...
        this.renameInputs.setPlaceholder(Component.translatable("gui.expatternprovider.renamer.input"));
        this.renameInputs.setResponder(s -> EPPNetworkHandler.INSTANCE.sendToServer(new CGenericPacket("set", s)));
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (this.renameInputs.isFocused() && keyCode == GLFW.GLFW_KEY_ENTER) {
            ...
            EPPNetworkHandler.INSTANCE.sendToServer(new CGenericPacket("set", this.renameInputs.getValue()));
            this.onClose();
            return true;
        }

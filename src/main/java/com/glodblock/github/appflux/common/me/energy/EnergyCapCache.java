...
public class EnergyCapCache {
    ...
    private final Map<Direction, BlockCapabilityCache<@NotNull IInWorldGridNodeHost, Void>> girdCache = new IdentityHashMap<>();
    ...
    @Nullable
    public <T> T getEnergyCap(BlockCapability<@NotNull T, Direction> cap, Direction side) {
    ...
    private BlockCapabilityCache<@NotNull IInWorldGridNodeHost, Void> getGridCache(Direction side) {
    ...
    private <T> BlockCapabilityCache<@NotNull T, Direction> getEnergyCacheInternal(BlockCapability<@NotNull T, Direction> cap, Direction side) {
        var capArr = this.cache.computeIfAbsent(cap, _ -> new BlockCapabilityCache[6]);
        ...
        return (BlockCapabilityCache<@NotNull T, Direction>) capArr[index];
    }

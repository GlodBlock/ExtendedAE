...
public class UniversalEnergyHandler {
    ...
    private static final Handler<EnergyHandler> DEFAULT = (accepter, storage, source) -> {
        try (var transaction = Transaction.openRoot()) {
            var toAdd = Math.min(accepter.getCapacityAsInt() - accepter.getAmountAsInt(), AFConfig.getFluxAccessorIO());
            if (toAdd > 0) {
                var drained = storage.getInventory().extract(FluxKey.of(EnergyType.FE), toAdd, Actionable.MODULATE, source);
                if (drained > 0) {
                    var actuallyDrained = accepter.insert((int) drained, transaction);
    ...
    public static <T> void addHandler(BlockCapability<@NotNull T, Direction> cap, Handler<T> handler) {
    ...
    public static <T> long send(@NotNull EnergyCapCache cache, Direction side, @NotNull IStorageService storage, @NotNull IActionSource source) {
        for (var entry : HANDLERS) {
            T cap = cache.getEnergyCap((BlockCapability<@NotNull T, Direction>) entry.left(), side);
            if (cap != null) {
                return ((Handler<T>) entry.right()).send(cap, storage, source);
            }
        }
        var cap = cache.getEnergyCap(Capabilities.Energy.BLOCK, side);
        if (cap != null) {

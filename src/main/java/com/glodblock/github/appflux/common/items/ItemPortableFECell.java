...
public class ItemPortableFECell extends AbstractPortableCell implements IFluxCell {
    ...
    public static final MenuType<@NotNull MEStorageMenu> FE_CELL_TYPE = MenuTypeBuilder
            .create(MEStorageMenu::new, IPortableTerminal.class)
            .buildUnregistered(AppFlux.id("portable_fe_cell"));

    public ItemPortableFECell(Properties properties, int quartKilobytes, double idleDrain, int defaultColor) {
        super(FE_CELL_TYPE, properties.stacksTo(1), defaultColor);
        ...
        if (entity instanceof Player player) {
            ...
            if (induct && storedInv != null) {
                ...
                for (int slot = 0; slot < inv.getContainerSize(); slot ++) {
                    ...
                    var cap = AFUtil.findCapability(inv.getItem(slot), Capabilities.Energy.ITEM);
                    if (cap != null) {
                        try (var trans = Transaction.openRoot()) {
                            var toAdd = cap.getCapacityAsInt() - cap.getAmountAsInt();
                            var canAdd = (int) storedInv.extract(key, toAdd, Actionable.MODULATE, IActionSource.ofPlayer(player));
                            if (canAdd > 0) {
                                var actualAdd = cap.insert(canAdd, trans);
                                var left = canAdd - actualAdd;
                                if (left > 0) {
                                    storedInv.insert(key, left, Actionable.MODULATE, IActionSource.ofPlayer(player));
                                }
                                trans.commit();
                            }
                        }
    ...
    @Override
    public void appendHoverText(ItemStack stack, TooltipContext context, TooltipDisplay tooltipDisplay, Consumer<Component> lines, TooltipFlag advancedTooltips) {
    ...
    @Override
    @Nullable
    public EnergyHandler getCapability(@NotNull ItemStack stack, ItemAccess access) {
        return new PoweredItemCapabilities(access, stack.getItem(), this);
    }

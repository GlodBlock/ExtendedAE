...
public record FluxNetworkCap(IStorageService storage, IActionSource source, IOSignal signal) implements IFNEnergyStorage {

    public static final BlockCapability<IFNEnergyStorage, Direction> CAP = FluxCapabilities.BLOCK;

    public static IFNEnergyStorage of(@Nullable IStorageService storage, IActionSource source, Supplier<EnergyIO> config) {
        if (storage != null) {
            return new FluxNetworkCap(storage, source, IOSignal.of(config));
        } else {

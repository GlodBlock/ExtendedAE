...
public class NetworkRender extends RenderType {
    ...
    public static void hook() {
        WorldRenderEvents.BEFORE_DEBUG_RENDER.register((listener) -> {
            PoseStack ps = listener.matrixStack();
            MultiBufferSource buffer = listener.consumers();
            Camera camera = listener.camera();
            var player = Minecraft.getInstance().player;
            if (player != null &&
                    player.getMainHandItem().getItem() == AEAItems.ANALYSER &&
                    buffer instanceof MultiBufferSource.BufferSource source) {
